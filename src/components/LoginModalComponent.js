...
const LoginModalComponent = ({account,setAccount, userBalance, setUserBalance, rewardAmt, setRewardAmt, reviveAmt, setReviveAmt, isLogin, setErrorMessage, isLoading, setIsLoading}) => {  
	...
	const checkContractForBalance = async (account) => {
		if (account) {
			try {
				...
				setUserBalance(Number(ethers.utils.formatEther(0)));
	...
	const queryTokenBalanceUpdated = async (oldBalance) => {
		...
		if (result!=null) Number(setUserBalance(result));
		setIsLoading(false);
		navigate("/")
		window.location.reload();
	}
  ...
  return (
		<ChakraProvider>
			...
			style={{
				...
				{account==='' ? 
					<button className='cs-button' type="button" onClick={connectButtonClickHandler}><Text color='white'>{connButtonText}</Text></button> 
				: 
					<>
						<Box className='accountDisplay'>
							<Text color='white' style={{cursor: 'default'}}>Address: {account}</Text>
						</Box>
					</>
				}
				{
